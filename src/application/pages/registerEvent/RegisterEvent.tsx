						...
						<div className='flex items-center'>
							...
							<div className='ml-3 flex-1'>
								...
								<p className='text-blue-700'>
									Te registraste con {currentRegistration.number_of_participants} participante(s) el{' '}
									{new Date(currentRegistration.created_at).toLocaleDateString()}
								</p>
							</div>
							{canCancel && (
								<button
				...
				{canRegister && (
					<div className='bg-white rounded-lg shadow-lg overflow-hidden'>
						<div className='px-6 py-4 border-b border-gray-200'>
							...
							<p className='text-gray-600 mt-1'>Completa la información para registrarte al evento</p>
						</div>
								...
								<div className='flex items-center justify-between'>
									...
									<span
										className={`text-sm font-semibold ${
											getAvailableSpots() > 0 ? 'text-green-600' : 'text-red-600'
										}`}>
										{getAvailableSpots()} cupos
									</span>
								</div>
								{isEventFull() && (
									<p className='text-sm text-red-600 mt-2'>Este evento está completamente lleno</p>
								)}
							</div>
