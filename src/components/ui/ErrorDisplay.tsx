...
const ErrorDisplay: React.FC<ErrorDisplayProps> = ({ error, onRetry, onDismiss, showDetails = false, className = '' }) => {
		...
		<div className={`border-l-4 p-4 rounded-r-lg ${getErrorColor(error.type)} ${className}`}>
			<div className='flex items-start'>
				...
				<div className='flex-1 min-w-0'>
					...
					<p className='mt-1 text-sm'>{ErrorService.getUserFriendlyMessage(error)}</p>
							...
							<ul className='text-xs space-y-1'>
								{error.details.map((detail, index) => (
									<li key={index} className='flex items-start'>
										{detail.field && <span className='font-medium mr-2'>{detail.field}:</span>}
