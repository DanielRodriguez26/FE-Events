...
const SessionSchedule: React.FC<SessionScheduleProps> = ({ sessions }) => {
	...
	const sortedDays = Object.keys(groupedSessions).sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
	...
	if (sessions.length === 0) {
		return (
			<div className='text-center py-8'>
				...
				<p className='mt-1 text-sm text-gray-500'>Aún no se han programado sesiones para este evento.</p>
			</div>
		);
	}
				...
				{sortedDays.map(day => (
					<div key={day} className='bg-white rounded-lg shadow-sm border'>
						<div className='px-6 py-4 border-b border-gray-200'>
							<h4 className='text-lg font-medium text-gray-900 capitalize'>{formatDate(day)}</h4>
						</div>
										...
										<div className='flex-1 min-w-0'>
											<div className='flex items-start justify-between'>
												<div className='flex-1'>
													<h5 className='text-lg font-medium text-gray-900 mb-1'>{session.title}</h5>
				...
				<div className='flex items-center'>
					...
					<div>
						...
						<p className='text-sm text-blue-700 mt-1'>
							El evento se desarrolla en {sortedDays.length} día{sortedDays.length !== 1 ? 's' : ''} con un total de{' '}
							{sessions.length} sesión{sessions.length !== 1 ? 'es' : ''}. Las sesiones están programadas entre las
							8:00 AM y las 10:00 PM con intervalos de 30 minutos.
						</p>
					</div>
				</div>
			</div>
